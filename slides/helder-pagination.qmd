...
Basicamente é relativamente caro mandar um monte de coisa de uma vez e correr o risco de corromper no caminho e ter que mandar de novo, e esse problema aumenta se forem muitos dados.
...
-   **Header link:** `Link:` to `next` in response `header`
    ...
    -   [Magic The Gathering Cards](https://docs.magicthegathering.io/#documentationheaders)
-   **Body link:** `nextUrl` in response `body`
    -   [open5e.com monsters](https://api.open5e.com/monsters/?limit=100) from D&D
:::
:::

-   shared: Most offer `perPage` or `pageSize` param
-   Header: É tipo onde estão as informações daquela API que vc hittou (tipo um cabeçalho mesmo, e as vezes o link da próxima página de resultados está nesse cabeçalho ou no conteúdo (body) da request
-   Muita das vezes vai ter um parâmetro que diz quantos resultados você consegue atingir com cada request e as vezes dá pra alterar esse valor.
...
Provalmente, quanto mais dinheiro envolvido nas request, mais eles tentarão te informar o quão "bonzinho" você precisa ser pra acessar pra fazer os requests e as paginações daquela API
...
-   `start`, `offset` = almost always leave as-is
    -   exemplo: começar na página 1 e iterar 1 página por vez (faz sentido pegar tudo, mas e se você quiser uma sample, pode usar esse parâmetro)
...
candidates_request <- 
  ...
  req_url_query(has_raised_funds = TRUE) # só pra diminuir o tamanho da resposta (n° de candidatos)
```
...
#> [1] 9
# ou seja, isso aqui são 9 responses diferentes, o que vamos vamos com isso é outro assunto, mas assim conseguimos 9 paginas completas. 
```
...
-   Isso aqui é provavelmente o que você precisa pra conseguir todos os dados de uma API
:::
