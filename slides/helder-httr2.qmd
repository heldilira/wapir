...
candidates <- 
  ...
  req_perform() |> # Chapter 5 (next), essa é a parte onde de fato estamos "hitting" o servidor
...
# estamos comparando as duas respostas, uma lida de um .json do computador e outra feita através de uma request do httr2, feita de forma pipe-based (o que facilita muito) 
```
...
-   "HyperText" = web content (textos e links, mas hoje em dia serve pra muito mais coisa, imagens, videos, dados, etc)
-   "Transfer" = exchange
-   "Protocol" = rules
-   "rules for exchanging web content"
-   HTTP(S) = most of internet communication (s = secure = seguro)
:::
...
# aqui estamos adicionando passo a passo uma nova informação para cada parte de uma request e reutilizando a parte anterior
```
:::
...
# Ou seja, fazer dessa forma possibilita vc alterar partes específicas de uma request, nesse caso pegamos a API e colocamos a parte de candidatos, mas que poderia ser outra, ou um vetor de outros endpoints, enfim. 
```
:::
...
# nesse caso ele dropou o /v1 e dessa forma não estaríamos acertando o endpoint correto, ou seja, ter cuidado com essa função pq ela "come" o URL anterior
```
:::
...
-   endpoint = candidates (na documentação a ideia é que tenha os parâmetros possíveis de atingir com o endpoint)

-   root/base URL = api.open.fec.gov/

-   path = v1/candidates/***?api_key=DEMO_KEY&office=P***

    -   tudo que vem depois de ? é query = ***?api_key=DEMO_KEY&office=P***
...
-   Aqui você provavelmente vai ter que experimentar pq normalmente os desenvolvedores não relatam muito bem qual a forma correta de pegar diferentes valores numa mesma URL.

-   As vezes eles não colocam na descrição da API mas tem na documentação da mesma.
...
# isso da erro pq: como vamos colocar vários parâmetros dentro do mesmo argumento? já que é um endereço só...use o .multi abaixo, ele disse que lá na especificação era um array, ou seja espera mais de algum valor. 
